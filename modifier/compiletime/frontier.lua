...
local function parse(regex, c, bs, ctx)
  ...
  if c == '%' then
    if base.raw.sub(regex, bs + 1, bs + 1) ~= 'f' then return end
    if base.raw.sub(regex, bs + 2, bs + 2) ~= '[' then error("missing '[' after '%f' in pattern") end
    ...
    class, nbs = ctx.parse(regex, '[', bs + 2, ctx)
    nbs = nbs + 2
